...
{      if(\Gate::denies('admin_staff')){
    ...
    //  dd($student);
	 ...
	 if($request->isMethod('post')){
            ...
            return redirect()->route('viewFees')->with('flash_message', 'New Service Charge Has Been Added');
...
public function viewFees(Request $request){
    ...
    $totalamountpaid = 0;
     ...
     if($request->isMethod('post')){
         ...
         if(!isset($request->student)){
             ...
             return back()->with('flash_error', 'Please Select Customer');
         }
    ...
    //   dd($fees[0]->title);
        ...
        $array=json_decode($fees[0]->title);
    ...
    //   dd($array);
        ...
        // dd($fees);
          return view ('admin.fee.view', compact('fees','students','student_name','totalamountpaid','array'));
        }
...
//    dd($payment);
 ...
 public function deletepayment($id){
    ...
    public function viewpaymentreport($id){
        ...
        // echo $totalpayment;
      return view('admin.invoices.sampleview',compact('lastpayment','student','totalamountpaid','paymentref'));
    }
 //generatePDF
    ...
    public function makepayment(Request $request){
        ...
        if($request->isMethod('post')){
             ...
             $count = DB::table('payments')
                ...
                ->count();
        $i=$count+1;
        $paymentref='IN'.date("Y/m/d").'/'.$i;
            $data=$request->all();

            $payment=new Payment;
            $payment->student_id=$data['student_id'];
            $payment->amount_paid=$data['amount'];
            $payment->payment_code=$paymentref;
            $payment->received_by=$data['received_by'];
            $payment->paid_by=$data['paid_by'];
            $payment->payment_title=$data['title'];
            $payment->mode_of_payment=$data['mode_of_payment'];
